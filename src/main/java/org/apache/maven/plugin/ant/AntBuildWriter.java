...
public class AntBuildWriter {
        ...
        if (extensionWriter.isBndProject()) {
            ...
            if (!bndFile.exists()) {
                ...
                if (instructions != null && instructions.length() > 0) {
                    ...
                    try {
                        writer.write("# Generated by maven-ant-plugin from in-line POM configurations\n\n");
    ...
    private void writeBuildPathDefinition(XMLWriter writer, String id, List artifacts) {
        ...
        if (extensionWriter.isBndProject() && !bndAntPresent && "build.classpath".equals(id)) {
            ...
            try {
                artifactResolverWrapper.getArtifactAbsolutePath("biz.aQute.bnd", "biz.aQute.bnd.ant", "7.4.0");
                writer.startElement("pathelement");
                writer.addAttribute(
                        "location", "${maven.repo.local}/" + artifactResolverWrapper.getLocalArtifactPath(bndArtifact));
                writer.endElement(); // pathelement
            } catch (IOException e) {
                // ignore
            }
